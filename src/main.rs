...
fn main() {
    ...
    let ssid: String<32> = String::from_str("myssid").unwrap();
    let password: String<64> = String::from_str("mypassword").unwrap();

    let peripherals = Peripherals::take().unwrap();
    let led_pin = PinDriver::output(peripherals.pins.gpio2).expect(
